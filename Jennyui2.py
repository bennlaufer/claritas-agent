...
# User Text Input and AI Agent Voice Output and Text Transcription
# Jennyui2.py
...
# --- TTS Function using Edge + AssemblyAI ---
def speak(text):
    async def run():
        communicate = edge_tts.Communicate(text, voice="en-US-AriaNeural")
        await communicate.save("output.mp3")
        os.system("afplay output.mp3")  # macOS playback; change to `playsound` or `subprocess` for Windows/Linux
    asyncio.run(run())
...
def run_ui():
    ...
    if user_input:
        ...
        speak(response["output"])  # Voice out via Edge TTS
